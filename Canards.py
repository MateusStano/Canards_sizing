...
class Compare:
    ...
    def calculateA(self, speed, stall_angle, fin_delta_angle, J, thetaDotDotMax, thetaDot0 = 0, DynamicPressure = 101325, compare = False, speedMin = 0.1, speedMax = 0.8, passo = 0.1, tempo = 3):
        ...
        return [A, Amin, speed]
    def torque(self, speed, XCgFins, stall_angle, Cr, span, Ct, X_t, rho = 1.06):
        # Condition to find the index at the panda dataset
        Airfoil1 = np.array(self.canard1.CN0)
        stall_angle = stall_angle * math.pi / 180
        speed = speed*343
        ...
        XFins = (X_t / 3) * ( (Cr + 2*Ct) / (Cr + Ct) ) + (1/6) * (Cr + Ct - (Cr * Ct) / (Cr + Ct))

        MaxTorque = MaxNormalForce * (XFins - XCgFins)

        print("Applied torque = {:.3f} N cm".format(MaxTorque*100))
        print("Applied torque = {:.3f} kgfcm".format(MaxTorque*100/9.81))
        print(XFins)
...
Comp.torque(speed = 0.4, XCgFins = 16.81/1000, stall_angle = alfa, Cr = Cr, span = s, Ct = Ct, X_t = X_t, rho = 1.06)